        ...
        <main class="main-content">
            <div class="isi">
                ...
                <div class="data">
                    ...
                    <form action="{{ route('jadwal-pelajaran.update', $jadwal->id_jadwal) }}" method="POST" class="p-4 pt-1 rounded-4 bg-white shadow-sm" id="formEditJadwal">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="id_mata_pelajaran" id="id_mata_pelajaran" class="form-select @error('id_mata_pelajaran') is-invalid @enderror" required>
                                    ...
                                    @foreach ($mataPelajaran as $mp)
                                        <option value="{{ $mp->id_mata_pelajaran }}" {{ old('id_mata_pelajaran', $jadwal->id_mata_pelajaran) == $mp->id_mata_pelajaran ? 'selected' : '' }}>
                                            {{ $mp->nama }} ({{ $mp->kode }})
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="session" id="session" class="form-select @error('session') is-invalid @enderror" required>
                                    ...
                                    @foreach ($timeSlots as $index => $slot)
                                        <option value="{{ $index + 1 }}" {{ old('session', $currentSession) == ($index + 1) ? 'selected' : '' }}>
                                            {{ $slot['label'] }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="id_tahun_ajaran" id="id_tahun_ajaran" class="form-select @error('id_tahun_ajaran') is-invalid @enderror" required>
                                    ...
                                    @foreach ($tahunAjaranList as $ta)
                                        <option value="{{ $ta->id_tahun_ajaran }}" {{ old('id_tahun_ajaran', $jadwal->id_tahun_ajaran) == $ta->id_tahun_ajaran ? 'selected' : '' }}>
...
<script>
    $(document).ready(function() {
        ...
        $('#id_kelas, #id_mata_pelajaran, #id_guru, #hari, #session, #status, #id_tahun_ajaran').select2({
            theme: 'bootstrap-5',
            width: '100%'
        });
        ...
        $('#btnCekRekomendasi').on('click', function() {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    id_kelas: idKelas,
                    hari: hari,
                    id_jadwal: {{ $jadwal->id_jadwal }} // Exclude current jadwal
                },
